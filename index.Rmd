...
# Warn if R version is older than what was used for the analysis
if (getRversion() < "4.5.0") {
  warning(
    "This analysis was developed under R 4.5.0. ",
    "You are using R ", as.character(getRversion()),
    ". If you see package errors, consider upgrading to R 4.5.0."
  )
}

# Check for required packages and fail with a clear message if missing
needed_pkgs <- c(
  "sf", "s2", "tidyverse", "dplyr", "ggplot2",
  "corrplot", "MBESS", "Metrics", "plyr", "tidyr", "tibble"
  # add or remove package names here if needed
)
...
if (length(missing_pkgs) > 0) {
  msg <- paste0(
    ...
    "Please run renv::restore() in the project root\n",
    "  (Gladish_2025_ReADI_Creation)\n",
    "before knitting this document.\n\n",
    "If renv::restore() fails with errors about 's2', 'cmake', or 'abseil',\n",
    "see the 'R and system requirements' section of the README for the\n",
    "one-time system commands to run on your operating system."
  )
  stop(msg, call. = FALSE)
}
```
